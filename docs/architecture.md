...
```json
{
  ...
  "structure": {
    ...
    "step_count": 0,
    "steps": [
      {"id": 1, "description": "string", "detail": "string (prose under a heading step; empty for list steps)", "location": "string (e.g. SKILL.md:42)", "tools": [], "references_steps": []}
    ],
    "inputs": ["string"],
    "outputs": ["string"],
    "dependencies": [
      {"from_step": 1, "to_step": 2, "detail": "string", "context": "string (wording around the mention)"}
    ],
    ...
    "unreferenced_scripts": ["string (executable nothing in the skill refers to — see note below)"],
    "decision_points": [
...
**2026-09-06 — `steps[].detail` (additive), and `description` drops the step label.** For a heading-based step, `description` was the full heading text including its label ("Step 4: Submit"), so every such step read as beginning with the word "Step" — no check on *how a step is worded* could ever see the real instruction, and the imperative check failed on all of them by construction. `description` is now the label-free title, and the prose beneath the heading is carried in `detail` (capped at 2000 chars, whitespace-normalised; empty for list-based steps, whose description is already the full instruction). Consumers judging wording should read description + detail; consumers measuring *length* or redundancy should keep using `description` alone, or a heading plus its whole section gets measured as one oversized instruction.

**2026-09-13 — `dependencies[].context` (additive), and `feedback_loops` no longer match a bare "iterate".** Two more corpus-measured false-positive sources. (1) A forward reference between steps was reported as a process defect on the numbers alone — but all 14 forward references across the corpus were cross-references ("see Step 4"), explanatory look-aheads ("Step 4 gives each surface its own section") or deliberate jumps ("move to step 4"), none of them defects. Telling those from a genuine ordering bug ("use the output from step 5") needs the wording, so each dependency now carries the surrounding text; Module 2 only reports a forward reference when it reads as consuming a later step's result, and stays silent when no context is supplied. (2) `feedback_loops` matched the bare verb "iterate", whose four corpus matches were all ordinary collection walks ("count, iterate or use pandas", "iterate page annotations") — two of which became HIGH "loop without an exit condition" findings. Only phrasings that describe going back round ("loop back", "repeat until", "return to step", "iterate until", …) now count.
