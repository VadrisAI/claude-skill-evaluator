...
const { readLatestHistory, writeHistoryEntry, compareScores } = require('./compare');
 ...
 * @param {string} [opts.version] - version label for this run (e.g. "v2"); falls back to evaluationOutput.version
 ...
 * @param {object} [opts.previousVersionScores] - explicit override for comparison; otherwise the latest history entry is used
...
function generateReport(evaluationOutput, opts = {}) {
  ...
  const previousEntry =
    opts.previousVersionScores !== undefined
      ? opts.previousVersionScores
      : readLatestHistory(outputDir);
  const previousVersionScores = previousEntry ? previousEntry.scores : null;
  const previousVersionLabel = previousEntry ? previousEntry.version : null;
